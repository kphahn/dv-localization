...
view_map = o3d.geometry.PointCloud()


def update_view(vis, pcd, frame, t):
...
if __name__ == "__main__":
    ...
    if VISUALIZATION:
        vis_map.create_window(width=1500, height=1000)
        vis_map.add_geometry(view_map)
    ...
    view_scene = o3d.geometry.PointCloud(lidar_dataset[0])

    vis_scene = o3d.visualization.Visualizer()
    vis_scene.create_window()
    vis_scene.add_geometry(view_scene)
    # vis.reset_view_point(True)

    view_control = vis_scene.get_view_control()
    view_control.camera_local_translate(186, 0, -44)
    theta = np.radians(-90)  # Convert x degrees to radians
    ...
    for run in range(5):
        for idx in range(0, len(lidar_dataset)):
            ...
            ### VISUALIZE SCENE
            update_view(vis_scene, view_scene, frame, "scene")
            ...
            if VISUALIZATION:
                ...
                update_view(vis_map, view_map, pose_graph.get_map() + pc, "map")
